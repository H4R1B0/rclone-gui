...
- **링크드 브라우징과의 상호작용**: back/forward는 **로컬 동작**. 링크드 브라우징이 켜져 있어도 상대 패널을 따라 이동시키지 않음. (링크드 브라우징은 "사용자 내비게이션에 반응"하는 사이드이펙트로 이미 loadFiles에 구현됨 — back에서 트리거하지 않기 위해 별도 경로로 구현)
