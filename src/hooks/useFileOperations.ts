...
export function useFileOperations(side: 'left' | 'right') {
  ...
  const copyToOtherPanel = useCallback(async () => {
    ...
    for (const name of selected) {
      ...
      if (file.IsDir) {
        await api().copyDir(panel.remote, srcPath, other.remote, dstPath);
      } else {
        await api().copyFile(panel.remote, srcPath, other.remote, dstPath);
      }
    }
  }, [side, panel.remote, panel.path, panel.files, panel.selectedFiles]);
  ...
  return { createFolder, deleteSelected, rename, copyToOtherPanel, moveToOtherPanel };
}
