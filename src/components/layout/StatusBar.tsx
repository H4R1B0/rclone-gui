...
export function StatusBar() {
  const { transfers, totalSpeed, errors, completed } = useTransferStore();
  const t = useT();
    ...
    <div className="flex items-center justify-between px-3 py-1 bg-surface-raised border-t border-border text-[11px] text-text-muted relative">
      ...
      <div className="flex items-center gap-3">
        ...
        {errors > 0 && (
          ...
          >
            <Bell size={12} />
            <span>{errors}{t('transfer.errorCount')}</span>
      ...
      {showErrors && errorList.length > 0 && (
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto">
            {errorList.map((e, i) => (
              <div key={`${e.name}-${i}`} className="px-3 py-2 border-b border-border/50">
