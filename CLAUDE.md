...
- 릴리즈 시 **DMG 패키징 후 GitHub Release에 업로드**한다

### 릴리즈 워크플로우

1. `project.yml`에서 `MARKETING_VERSION` 업데이트
2. `xcodegen generate`로 프로젝트 재생성
3. Release 아카이브 빌드: `xcodebuild -scheme RcloneGUI -configuration Release -arch arm64 ONLY_ACTIVE_ARCH=YES -archivePath /tmp/RcloneGUI-vX.Y.Z.xcarchive archive`
4. DMG 생성: 아카이브의 `.app`을 `/Applications` 심링크와 함께 `hdiutil create`
5. Git 태그 생성 + push: `git tag vX.Y.Z && git push origin vX.Y.Z`
6. GitHub Release 생성 (한국어/영어 이중 표기 형식)
7. DMG 파일 업로드: `gh release upload vX.Y.Z RcloneGUI-vX.Y.Z.dmg`
